...
with data as (
	...
	select dateadd(month, 1, to_date(date)) date,0 as  new_cases, 0 as new_active_cases, 0 as new_recovered, 0 as new_deaths, sum(new_cases) prev_week_new_cases, sum(new_active_cases) prev_week_new_active_cases, sum(new_recovered) prev_week_new_recovered, sum(new_deaths) prev_week_new_deaths
	from FIVETRAN_INTERVIEW_DB.GOOGLE_SHEETS.COVID_19_INDONESIA_ADITYA_ARYAN
	group by date)
	group by date
)
